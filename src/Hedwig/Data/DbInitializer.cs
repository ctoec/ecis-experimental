...
namespace Hedwig.Data
{
  public static class DbInitializer
  {
    public static void Initialize(HedwigContext context)
    {
      ...
      var fundingSpaces = new FundingSpace[]
      {
        new FundingSpace { Capacity = 10, OrganizationId = organizations[0].Id, Source = FundingSource.CDC, AgeGroup = Age.Infant, Time = FundingTime.Full },
        new FundingSpace { Capacity = 20, OrganizationId = organizations[0].Id, Source = FundingSource.CDC, AgeGroup = Age.Preschool, Time = FundingTime.Full }
      };

      foreach (FundingSpace f in fundingSpaces)
      {
        context.FundingSpaces.Add(f);
      }
      context.SaveChanges();
      ...
      var children = new Child[] {
        new Child { FirstName = "Alan", LastName = "Rickman",
          ...
          BirthCertificateId = "XXX-XXX-XXXX"
        },
