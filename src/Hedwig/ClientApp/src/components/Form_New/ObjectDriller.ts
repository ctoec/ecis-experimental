...
export class ObjectDriller<T> {
	 ...
	 */
	at<K extends keyof T>(field: K): ObjectDriller<
		NonNullable<T[K]>
	> {
		if (this.value == undefined) { // checks null or undefined
	...
	find<S extends any[], K extends keyof S>(func: (_:S[number]) => boolean): ObjectDriller<NonNullable<S[K]>>
	{
