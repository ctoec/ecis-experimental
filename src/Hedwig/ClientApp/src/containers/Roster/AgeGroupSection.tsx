...
import { generateFundingTag, isCurrentToRange, NO_FUNDING } from '../../utils/models';
	...
	columns: [
		{
			...
			cell: ({ row }) => (
				<th scope="row">
					...
					&nbsp;
					{(isFunded(row, { source: FundingSource.CDC, currentRange: { startDate: row.entry, endDate: row.exit}  }) && hasValidationErrors(row)) ? InlineIcon({ icon: 'incomplete' }) : ''}
				</th>
			),
			...
			cell: ({ row }) => (
				<td>
					{row.fundings && row.fundings.length > 0 ?
						row.fundings
						.filter(funding => isCurrentToRange(funding, {startDate: row.entry, endDate: row.exit}))
