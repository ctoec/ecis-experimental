 ...
 */
export function getEnrollmentHistoryProps(currentEnrollment: DeepNonUndefineable<Enrollment>): ProcessStepProps[] {
	...
	allEnrollments.forEach((enrollment, idx) => {
		...
		if (enrollment.entry) {
			...
			processStepProps.push(getSortableStep({
				heading: idx === allEnrollments.length - 1 
		...
		// funding steps
		processStepProps = processStepProps.concat(fundingStepProps(enrollment.fundings, enrollment.exit));

		// only add determination steps and C4K funding steps for the current enrollment (others will be dupes)
		if(idx === 0) {
			// determination steps
			processStepProps = processStepProps.concat(determinationStepProps(enrollment.child.family.determinations))
 ...
 */
function fundingStepProps(fundings: DeepNonUndefineableArray<Funding> | null, enrollmentExit: Date | null) {
	var processStepProps: SortableProcessStepProps[] = [];
	...
	if(fundings) {
		fundings.forEach((funding) => {
			...
			else if (funding.source === FundingSource.CDC
				&& funding.lastReportingPeriod
				&& !enrollmentExit) {
				var dayAfterLastReportingPeriodEnd = moment(funding.lastReportingPeriod.periodEnd).add(1, 'days').toDate();
				processStepProps.push(getSortableStep({
 ...
 */
function determinationStepProps(determinations: DeepNonUndefineableArray<FamilyDetermination> | null) {
	var processStepProps: SortableProcessStepProps[] = [];
	if(determinations) {
