...
export class DeepNonUndefineableArray<T> extends Array<DeepNonUndefineable<T>> {
	...
	filter<S extends DeepNonUndefineable<T>>(
		callbackfn: (
			value: DeepNonUndefineable<T>,
			index: number,
			array: DeepNonUndefineable<T>[]
		) => boolean,
		thisArg?: any
	): DeepNonUndefineable<T[]> {
		return super.filter<S>(((obj, i, arr) => callbackfn(obj, i, arr)) as (
			_: DeepNonUndefineable<T>,
			i: number,
			a: DeepNonUndefineable<T>[]
		) => _ is S);
	}

	find<S extends DeepNonUndefineable<T>>(
		predicate: (
			value: DeepNonUndefineable<T>,
			index: number,
			obj: DeepNonUndefineable<T>[]
		) => boolean,
		thisArg?: any
	): DeepNonUndefineable<T> | undefined {
		return super.find<S>(((val, i, o) => predicate(val, i, o)) as (
			_: DeepNonUndefineable<T>,
			i: number,
			o: DeepNonUndefineable<T>[]
		) => _ is S);
	}
}
...
export type DeepNonUndefineable<T> = T extends any[]
	? DeepNonUndefineableArray<T[number]>
	: T extends (...args: any[]) => any
	? FunctionWithRequiredReturnType<T>
	: T extends object
	? DeepNonUndefineableObject<T>
	: NonUndefineable<T>;
