...
namespace HedwigTests.Repositories
{
  ...
  bool includeDeterminations
    ...
    [InlineData(false, new string[] { "fundings" })]
    [InlineData(true, new string[] { })]
    [InlineData(true, new string[] { "fundings" })]
    public async Task Get_Enrollments_For_Organization_Report(
      bool submitted,
      string[] include
    )
    {
      using (var context = new TestContextProvider().Context)
      {
        ...
        var enrollmentResult = result.FirstOrDefault();
        ...
        {
          // It uses the current state of the Enrollment
          Assert.Equal(Age.School, enrollmentResult.Age);
        }
        ...
        if (include.Contains("fundings"))
        {
          ...
          {
            // It uses the current state of the Funding
            Assert.Equal(FundingTime.Part, fundingResult.Time);
          }
        }
      }
    }
