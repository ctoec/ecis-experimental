...
namespace HedwigTests.Repositories
{
	public class EnrollmentRepositoryTests
	{
		...
		[Fact]
		public async Task Get_Enrollments_By_Site_Id_With_Incomplete()
		{
			using (var context = new TestContextProvider().Context)
			{
				// If an enrollment exist in a site with 
				...
				// and exit date of 06-01-2019
				var site = SiteHelper.CreateSite(context);
				var enrollment1 = EnrollmentHelper.CreateEnrollment(context, site: site);
				enrollment1.Entry = new DateTime(2019, 1, 1, 0, 0, 0);
				enrollment1.Exit = new DateTime(2019, 6, 1, 0, 0, 0);
				var enrollment2 = EnrollmentHelper.CreateEnrollment(context, site: site);
				enrollment2.Entry = null;
				enrollment2.Exit = null;
				context.SaveChanges();

				// When an enrollment repository is queried with a site id
				var enrollmentRepo = new EnrollmentRepository(context);
				var res = await enrollmentRepo
					.GetEnrollmentsBySiteIdsWithIncompleteAsync(new int[] { site.Id });
				...
				Assert.Equal(new int[] { enrollment1.Id, enrollment2.Id }, enrollmentIds);
			}
		}

		[Fact]
		public async Task Get_Enrollments_By_Site_Id_Excludes_Incomplete()
		{
			using (var context = new TestContextProvider().Context)
			{
				// If an enrollment exist in a site with 
				...
				enrollment.Entry = new DateTime(2019, 1, 1, 0, 0, 0);
				enrollment.Exit = new DateTime(2019, 6, 1, 0, 0, 0);
				var emptyEnrollment = EnrollmentHelper.CreateEnrollment(context, site: site);
				emptyEnrollment.Entry = null;
				emptyEnrollment.Exit = null;
				context.SaveChanges();
